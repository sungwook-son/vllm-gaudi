...
ALLOWED_SCALE_METHODS=("unit_scale" "unit_scale"  "maxabs_hw" "maxabs_pow2" "maxabs_hw_opt_weight" "act_maxabs_pow2_weights_pcs_opt_pow2" "act_maxabs_hw_weights_pcs_maxabs_pow2" "act_maxabs_pcs_pow2_weight_maxabs_pts_pow2_hw")
usage() {
    echo
    ...
    echo "  -s    - scale method of quantization (default: maxabs_hw) "
    echo
}
...
create_measure_config() {
    ...
    if [[ $model_name_lower =~ ^mixtral ]]; then
        tmp_config="{\"method\": \"HOOKS\",\"mode\": \"MEASURE\",\"observer\": \"maxabs\",\"allowlist\": {\"types\": [], \"names\":  []},\"blocklist\": {\"types\": [], \"names\":  [\"self_attn\", \"lm_head\"]},\"quantize_weight\": false,\"measure_exclude\": \"NONE\",\"dump_stats_path\": \"$1/$2/$3/inc_output\",\"calibration_sample_interval\": 1}"
    elif [[ $model_name_lower =~ ^deepseek ]]; then
        tmp_config="{\"method\": \"HOOKS\",\"mode\": \"MEASURE\",\"observer\": \"maxabs\",\"allowlist\": {\"types\": [], \"names\":  []},\"blocklist\": {\"types\": [], \"names\":  [\"lm_head\", \"mlp\\\.gate\\\b\"]},\"quantize_weight\": false,\"measure_exclude\": \"NONE\",\"dump_stats_path\": \"$1/$2/$3/inc_output\",\"calibration_sample_interval\": 1}"
    else
        tmp_config="{\"method\": \"HOOKS\",\"mode\": \"MEASURE\",\"observer\": \"maxabs\",\"allowlist\": {\"types\": [], \"names\":  []},\"blocklist\": {\"types\": [], \"names\":  []},\"quantize_weight\": false,\"measure_exclude\": \"NONE\",\"dump_stats_path\": \"$1/$2/$3/inc_output\",\"calibration_sample_interval\": 1}"
    fi
...
create_quant_config() {
    ...
    if [[ ! " ${ALLOWED_SCALE_METHODS[*]} " =~ " $SCALE_METHOD " ]]; then
        echo "Invalid scale method: $SCALE_METHOD. Allowed scale method: ${ALLOWED_SCALE_METHODS[*]}"
        exit 1
    fi
    ...
    if [[ $model_name_lower =~ ^mixtral ]]; then
        if [[ $PT_HPU_LAZY_MODE == 0 ]]; then
            tmp_config="{\"mode\": \"QUANTIZE\",\"observer\": \"maxabs\",\"scale_method\": \"${SCALE_METHOD}\", \"scale_format\": \"CONST\",\"allowlist\": {\"types\": [],\"names\": []},\"blocklist\": {\"types\": [],\"names\": [\"self_attn\", \"lm_head\"]},\"dump_stats_path\": \"$1/$2/$3/inc_output\"}"
        else
            tmp_config="{\"mode\": \"QUANTIZE\",\"observer\": \"maxabs\",\"scale_method\": \"${SCALE_METHOD}\",\"allowlist\": {\"types\": [],\"names\": []},\"blocklist\": {\"types\": [],\"names\": [\"self_attn\", \"lm_head\"]},\"dump_stats_path\": \"$1/$2/$3/inc_output\"}"
        fi
    elif [[ $model_name_lower =~ ^deepseek ]]; then
        tmp_config="{\"mode\": \"QUANTIZE\",\"observer\": \"maxabs\",\"scale_method\": \"${SCALE_METHOD}\", \"scale_format\": \"scalar\", \"allowlist\": {\"types\": [],\"names\": []},\"blocklist\": {\"types\": [],\"names\": [\"lm_head\", \"mlp\\\.gate\\\b\"]},\"dump_stats_path\": \"$1/$2/$3/inc_output\"}"
    else
        tmp_config="{\"mode\": \"QUANTIZE\",\"observer\": \"maxabs\",\"scale_method\": \"${SCALE_METHOD}\",\"allowlist\": {\"types\": [],\"names\": []},\"blocklist\": {\"types\": [],\"names\": []},\"dump_stats_path\": \"$1/$2/$3/inc_output\"}"
    fi
...
while getopts "m:b:l:t:d:h:o:r:s:ue" OPT; do
    case ${OPT} in
        m )
            MODEL_PATH="$OPTARG"
            ;;
...
if $SKIP_STEP_1; then
    ...
    echo ""
    echo "1/5 Preparing calibration dataset"
...
echo ""
echo "3/5 Postprocessing scales"
...
echo ""
echo "4/5 Postprocessing b2b scales"
python3 step-4-postprocess-b2b-scales.py -m $FP8_DIR/$MODEL_NAME/$DEVICE_TYPE/ -o inc_b2b_tmp/$MODEL_NAME/$DEVICE_TYPE/  $EXTRA_FLAGS_STEP_3 || (echo "Error in step 4" && exit 1)
cp inc_b2b_tmp/$MODEL_NAME/$DEVICE_TYPE/* $FP8_DIR/$MODEL_NAME/$DEVICE_TYPE/
echo "Step 4/5 done"
...
echo ""
echo "5/5 Quantize scales"
if $MULTI_NODE_SETUP; then
    env $EXTRA_ENVS_STEP_4 python3 step-5-quantize-scales.py --model $MODEL_PATH --tensor-parallel-size $TP_SIZE --distributed-executor-backend ray $EXTRA_FLAGS_STEP_4 || (echo "Error in step 4" && exit 1)
else
    env $EXTRA_ENVS_STEP_4 python3 step-5-quantize-scales.py --model $MODEL_PATH --tensor-parallel-size $TP_SIZE $EXTRA_FLAGS_STEP_4 || (echo "Error in step 4" && exit 1)
fi

if [[ -n $RANK ]]; then
    echo ""
    echo "6/6 Unify scales"
    QUANT_DIR=$FP8_DIR/$MODEL_NAME/$DEVICE_TYPE/
    python3 step-6-unify_measurements.py -r $RANK -m $QUANT_DIR -o $QUANT_DIR $USE_EP || (echo "Error in step 5" && exit 1)
    echo "Step 6/6 done"
fi
cleanup_tmp
